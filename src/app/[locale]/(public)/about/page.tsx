  ...
  return (
    <div className="min-h-screen flex flex-col bg-background overflow-x-hidden selection:bg-ocean-cyan/20">
      ...
      <main className="flex-grow max-w-[1200px] mx-auto w-full px-6 py-16 space-y-24">
        ...
        <section className="text-center max-w-3xl mx-auto animate-in fade-in duration-700 delay-300">
        ...
        <section>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {[
              { id: 'transparency', icon: History, color: 'text-ocean-cyan', bg: 'bg-ocean-cyan/10' },
              { id: 'efficiency', icon: Zap, color: 'text-ucn-gold', bg: 'bg-ucn-gold/10' },
              { id: 'collaboration', icon: Users, color: 'text-purple-500', bg: 'bg-purple-500/10' }
            ].map((card, idx) => {
              const Icon = card.icon;
              return (
                <div key={idx} className="bg-card/50 backdrop-blur-sm border border-border/50 p-8 rounded-3xl text-center hover:shadow-xl hover:border-ocean-cyan/30 hover:bg-card transition-all duration-300 cursor-default group">
        ...
        <section className="pt-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-6">
            {[
              ...
              <a key={idx} href={tech.url} target="_blank" rel="noopener noreferrer" className="border border-border/50 p-6 rounded-3xl flex flex-col items-center justify-center text-center bg-card/30 backdrop-blur-sm hover:bg-card hover:border-ocean-cyan/30 hover:shadow-lg hover:-translate-y-1 transition-all duration-300 group">
                <Image 
                  src={tech.iconUrl} 
                  ...
                  height={48}
                  unoptimized
                  className={`w-12 h-12 mb-4 opacity-80 group-hover:opacity-100 transition-opacity ${tech.invertDark ? 'dark:invert' : ''}`} 
                />
                <span className="font-bold text-foreground text-sm mb-1 group-hover:text-ocean-cyan transition-colors">{tech.name}</span>
        ...
        <section className="pt-8 pb-12">
          ...
          <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto">
            {[
              ...
              <div key={idx} className="p-8 bg-card/50 backdrop-blur-sm border border-border/50 rounded-[2rem] shadow-sm flex flex-col justify-between hover:shadow-xl hover:border-ocean-cyan/30 transition-all duration-300 group relative overflow-hidden">
                <div className="absolute top-0 inset-x-0 h-24 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-ocean-cyan/20 via-muted to-muted rounded-t-[2rem]"></div>
                ...
                <div className="flex flex-col items-center mb-6 relative z-10 pt-4">
                  <Image 
                    ...
                    height={112}
                    unoptimized
                    className="w-28 h-28 rounded-full border-4 border-card shadow-md mb-5 object-cover group-hover:scale-105 transition-transform bg-muted" 
                  />
                  ...
                  <span className="text-xs font-bold uppercase tracking-wider text-ocean-cyan bg-ocean-cyan/10 px-3 py-1 rounded-full mt-4 text-center">{dev.minor}</span>
                </div>
                
                <div className="mt-4 pt-6 border-t border-border flex items-center justify-center gap-4">
                  <a href={dev.github} target="_blank" rel="noopener noreferrer" className="flex-1 flex items-center justify-center gap-2 py-3 bg-muted/50 hover:bg-muted rounded-xl text-foreground/80 hover:text-foreground transition-colors text-sm font-bold border border-transparent hover:border-border/50">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-4 h-4"><path d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3 0 6-2 6-5.5.08-1.25-.27-2.48-1-3.5.28-1.15.28-2.35 0-3.5 0 0-1 0-3 1.5-2.64-.5-5.36-.5-8 0C6 2 5 2 5 2c-.3 1.15-.3 2.35 0 3.5A5.403 5.403 0 0 0 4 9c0 3.5 3 5.5 6 5.5-.39.49-.68 1.05-.85 1.65-.17.6-.22 1.23-.15 1.85v4"></path><path d="M9 18c-4.51 2-5-2-7-2"></path></svg>
                    <span>GitHub</span>
                  </a>
                  <a href={dev.linkedin} target="_blank" rel="noopener noreferrer" className="flex-1 flex items-center justify-center gap-2 py-3 bg-ocean-cyan/10 hover:bg-ocean-cyan/20 rounded-xl text-ocean-cyan transition-colors text-sm font-bold border border-transparent hover:border-ocean-cyan/20">
